...
public class DriveTrain extends PIDSubsystem {
	...
	public void driveJoysticks() {
		...
		if (Robot.driveStickLeft.getY() > 0.005) {
			Robot.driveRobot.arcadeDrive(-Robot.driveStickLeft.getY() * multiplier - 0.28,Robot.driveStickLeft.getX());
	...
	public double modifiedInput(double input) { //restrains input value to -1 to 1
		double newInput;
		if (input > 1) {
			newInput = 1;
		} else if (input < -1) {
	...
	public double gyroRate() { //returns rotational velocity in degrees per second
