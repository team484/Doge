...
public class TotePickup extends Subsystem {
    ...
    public boolean toteDownTo0High() {
    	if (Robot.totePickup0High.get() != RobotMap.halleffectDefault && Robot.totePickupBottom.get() == RobotMap.halleffectDefault) {
    		Robot.totePickupMotor.set(0);
    		return true;
    	} else {
    		Robot.totePickupMotor.set(-1 * RobotMap.winchSpeedMultiplier);
    ...
    public boolean toteDownTo1High() {
    	if (Robot.totePickup1High.get() == RobotMap.halleffectDefault && Robot.totePickup0High.get() == RobotMap.halleffectDefault && Robot.totePickupBottom.get() == RobotMap.halleffectDefault) {
    		Robot.totePickupMotor.set(-1 * RobotMap.winchSpeedMultiplier);
    		return false;
    	} else {
    		Robot.totePickupMotor.set(0);
