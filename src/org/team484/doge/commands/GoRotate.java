...
public class GoRotate extends Command {
	...
	protected void initialize() {
		pid.reset();
		// pid.setPID(SmartDashboard.getNumber("Kp", 0.0),
		// SmartDashboard.getNumber("Ki", 0.0), SmartDashboard.getNumber("Kd",
		// 0.0));
		pid.enable();
	}
	...
	protected boolean isFinished() {
		if (Math.abs(Robot.driveTrain.gyroAngle() - pid.getSetpoint()) < 2.5) {
			return pid.onTarget();
		} else if (Robot.driveTrain.gyroRate() < 3
				&& Math.abs(Robot.driveTrain.gyroAngle() - pid.getSetpoint()) < 2.5) {
			return true;
		} else if (Robot.frontLeftMotor.get() < 0.01
				&& Robot.rearLeftMotor.get() < 0.01
				&& Robot.frontRightMotor.get() < 0.01
				&& Robot.rearRightMotor.get() < 0.01
				&& Math.abs(Robot.driveTrain.gyroAngle() - pid.getSetpoint()) < 5) {
			return true;
		} else {
			return false;
		}
